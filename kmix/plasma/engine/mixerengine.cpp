    ...
    m_kmix(0)
{
    ...
    connect(
        watcher, SIGNAL(serviceRegistered(QString)),
        this, SLOT(slotServiceRegistered(QString))
    );
    connect(
        watcher, SIGNAL(serviceUnregistered(QString)),
...
MixerInfo* MixerEngine::createMixerInfo(const QString& dbusPath)
{
    ...
    curmi->iface = new OrgKdeKMixMixerInterface(KMIX_DBUS_SERVICE, dbusPath, QDBusConnection::sessionBus(), this);
...
ControlInfo* MixerEngine::createControlInfo(const QString& mixerId, const QString& dbusPath)
{
    ...
    curci->iface = new OrgKdeKMixControlInterface(KMIX_DBUS_SERVICE, dbusPath, QDBusConnection::sessionBus(), this);
...
void MixerEngine::getInternalData()
{
    ...
    if (!m_kmix) {
        m_kmix = new OrgKdeKMixMixSetInterface(
            KMIX_DBUS_SERVICE, KMIX_DBUS_PATH,
